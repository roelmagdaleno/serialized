...
| Escaped strings | `S:` strings, as a value and as an array key, convert to what their escapes spell; one inside a `C:` body is rewritten with the body's length recalculated; no `E_DEPRECATED` reaches the caller; a payload without `S:`, or with `S:` only inside an `s:` value, reaches `unserialize()` byte for byte |
